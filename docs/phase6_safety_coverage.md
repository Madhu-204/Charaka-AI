...
- **Answer-layer hardening (2026-08-30):** `synthesis.py` now forwards `verification_notes` + `source_disagreements` into the LLM context (previously only `safety_flags` reached synthesis) — so species/identity closest-match disclosures and kataka's nux-vomica disambiguation, plus the previously-invisible strong-caution `source_disagreements`, now render in the answer. **Live-verified** for reachable herbs (kataka, arka, padmakesara — see `phase6_report.md §7b`).
- **Reachability diagnostic (2026-08-30):** 22 of 92 safety entries are unreachable at runtime (no verse mention in the ingested 20-chapter subset). Raw-text search of all 2,490 verses classifies them **10 alias-gap** (eranda←castor, nagarmotha←musta, trikatu←three spices, rasna←vanda, ketaki←screw pine, katphala←box myrtle, raktachandana←red sandal, surasa←holy basil, fenugreek←methi, atibala←country mallow — fix = alias/merge) and **12 corpus-gap** (jivanti, chochchika, priyangu, nagakesara, shalparni, stira, gokarna, brihati, bibhitaki, daruharidra, ajwain, yavani — genuinely absent from the ingested subset; fixing = **Phase 11 corpus expansion**, not a bug). `audit_safety_coverage.py` now runs a **runtime-reachability** check (reports `runtime-reachable: 70/92` + the 22, advisory) so future fill batches surface the gap automatically. README updated so coverage claims match user-reachable reality.