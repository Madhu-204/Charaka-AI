...
Example, `ashwagandha` (live-tool output verified; claims scoped to the fetched NCCIH page text):
- Contraindications: autoimmune disorders, thyroid disorders, not recommended for people about to have surgery, rare liver-injury cases, hormone-sensitive prostate cancer, drowsiness + GI upset (all NCCIH-page-stated — the "2 weeks before surgery" timeframe and "stop if dark urine/jaundice" instruction are **not** on the page and were removed in the batch-#1 correction, §6.4-10)
- Interactions (NCCIH-listed medicines only, mechanism guesses trimmed): diabetes, high-blood-pressure, immunosuppressant, sedative, anticonvulsant, thyroid-hormone medication interactions
...
8. **Cross-verified batch #1 + source-disagreement flagging.** 3 herbs stamped `modern_source_verified: true` after a cross-check pass against NCCIH fact sheets: **ashwagandha, turmeric, liquorice**. **Correction (same day):** the first version of these entries contained several claims *not* on the cited pages — including the ashwagandha "2 weeks before surgery" and "stop if dark urine/fatigue/jaundice" specifics — so all three entries were **re-scoped to exact page text** with a full trim/log in `docs/phase6_safety_coverage.md` §5. `modern_source_verified: true` now means *claims == fetched page text*. Added `source_disagreements`: for *verified* entries carrying a strong modern caution (hard pregnancy avoid or toxicity), a practitioner-review note is emitted by rule — smoke-tested to flag ashwagandha + liquorice and correctly **not** turmeric.
...
10. **Verification transparency + batch-#1 correction (recorded honestly, revised 2026-08-29).** Of the three batch-#1 flips, only **liquorice** was written against an NCCIH page actually fetched in-thread at the time; the ashwagandha/turmeric cautions were initially applied from model recall of NCCIH content. A direct re-fetch (2026-08-29) then disproved part of the earlier "verbatim-confirmed" claim: the ashwagandha specifics **"2 weeks before surgery"** and **"stop if dark urine/fatigue/jaundice"** (the latter is on the *turmeric* page, not the ashwagandha page) are absent from the ashwagandha page. Re-inspection of all three entries found the same non-page claim class elsewhere: turmeric's gallstones/bleeding-disorder contraindications and four interactions, and liquorice's hypokalemia/edema contraindications and digoxin/diuretic/antihypertensive/anticoagulant interactions — none stated on the respective pages. **Resolution:** all three entries were re-scoped so every contraindication/interaction/pregnancy claim matches the fetched page text (fetched-and-stamped 2026-08-29); removed claims (correct general pharmacology, but not NCCIH statements) are logged in `docs/phase6_safety_coverage.md` §5 for return via a separately-checked second source. `modern_source_verified: true` is now defined as *claims == fetched page text*, and batch #2+ enforces fetch-before-flip.
...
- [x] Cross-verify entries against NCCIH — **batch #1 done (3/51)**: ashwagandha, turmeric, liquorice. **Batch #1 correction** (2026-08-29): entries re-scoped to exact fetched page text after direct re-fetch found non-page claims (§6.4-10; `phase6 §5`)
...
**1. Was NCCIH actually fetched or generated? — Honest answer: mixed at write time, then re-verified by direct re-fetch, which caught a residual fabrication.**
Of batch #1's three `modern_source_verified: true` flags: **liquorice** was written against the NCCIH page actually fetched in-session; **ashwagandha** and **turmeric** caution-additions were applied from model recall of NCCIH content and were **not** yet fetch-confirmed at commit `a6c07db`. The reviewer's fabricated-verification risk was real for two of three flags. Resolution 1: both pages were re-fetched 2026-08-29 and apparent confirmations stamped. **Resolution 2 (corrects Resolution 1):** a further direct fetch of the ashwagandha page proved that two specifics that had been called "verbatim-confirmed" — the "2 weeks before surgery" timeframe and the "stop if dark urine/fatigue/jaundice" endpoint — are **not** on that page at all. The same pass found the identical non-page claim class in turmeric and liquorice. Fix: all three entries were re-scoped to match the fetched pages exactly, `modern_source_verified: true` redefined as *claims == fetched page text* (fetched same-day), and the removed claims logged in `docs/phase6_safety_coverage.md` §5. Full provenance in §6.4-10.
...
**Phase 5 verdict: CLOSED.** Every question the reviewer needed answered is answered with evidence committed, including the batch-#1 verification correction (§6.4-10) — the verification flag has been cleaned so `modern_source_verified: true` means *claims == fetched page text*, which is the baseline batch #2+ builds on. What remains for Phase 6 is data completeness + verification integrity, not new engineering.

**Reviewer's pulled-forward Phase-6 items — status:**

| # | Item | State |
|---|---|---|
| 1 | Verify the verification (were NCCIH pages actually fetched?) | ✅ Done — mixed-at-write → direct re-fetch → residual fabrication found (§6.4-10) → all three entries re-scoped to exact page text; `verified` == *claims == page text*; fetch-before-flip for batch #2+ |
...
| 5 | Cross-verification batch #2 (API of India: brahmi, guggulu, shatavari, …) | ⬜ Queued next — fetch-before-flip, claims == page text |
